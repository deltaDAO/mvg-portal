...
const WordCloud: React.FC<WordCloudProps> = ({ skipLoading = false, data }) => {
  console.log('WordCloud component received data prop:', data)
  // Get theme from context
  const { theme } = useTheme()

  // Get data states from useWordCloudData hook
  const { words, isLoading, error } = useWordCloudData({ data })

  // Get UI states and actions from useWordCloudStore
  const {
    // Data states (filteredWords comes from store, derived from words)
    filteredWords,
    ...
    // Actions (exclude fetchData, as it's now in useWordCloudData)
    setSearchTerm,
    setMinFrequency,
    setMaxWords,
    setDimensions,
    ...
    filterWords, // Keep filterWords as it operates on store state
    autoDetectStopwords,
    getActiveStopwords,
    getActiveWhitelist,
    getWordColor
  ...
  // Derive modalsOpen state locally
  const modalsOpen =
  ...
  useEffect(() => {
    ...
    // Also trigger filterWords here to ensure filters are applied when words data changes
    if (words.length > 0) {
      // Only filter if there are words
      filterWords()
    }
    resetZoom()
  }, [
    ...
    words,
    filterWords
  ]) // Add words and filterWords to dependencies

  // Auto-detect stopwords and filter words when words data is loaded
  useEffect(() => {
    if (words.length > 0) {
      autoDetectStopwords()
      filterWords()
    }
  }, [words, autoDetectStopwords, filterWords]) // Add dependencies

  // Add a dependency on isLoading to trigger filterWords when loading finishes
  useEffect(() => {
    if (!isLoading && words.length > 0) {
      filterWords()
    }
  }, [isLoading, words, filterWords]) // Add dependencies

  // Side effect to manage isUpdating state in the store
  useEffect(() => {
    if (
      isLoading ||
      (words.length > 0 && filteredWords.length === 0 && !error)
    ) {
      setIsUpdating(true)
    } else {
      setIsUpdating(false)
    }
  }, [isLoading, words, filteredWords, error, setIsUpdating])
  ...
  // Show a message if there are no filtered words
  if (!isLoading && !error && filteredWords.length === 0) {
    return (
      <div className="min-h-[500px] flex items-center justify-center text-gray-500 dark:text-gray-400">
        No words to display based on current filters.
      </div>
    )
  }
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        >
          <div className="h-[550px] bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 shadow-md flex items-center justify-center p-4 overflow-hidden relative wordcloud-container">
            ...
            {error ? (
              <ChartError message={error} onRetry={filterWords} />
            ) : isUpdating ? (
              <div className="absolute inset-0 bg-white/50 dark:bg-gray-900/50 flex items-center justify-center z-10">
                <div className="flex items-center space-x-2">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500 dark:border-blue-400"></div>
