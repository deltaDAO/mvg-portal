...
// Move getSentimentColor function to the top to fix "use before define" error
...
// Fix debounce function to handle Promise properly
...
const SentimentChart = ({ skipLoading = false }: SentimentChartProps) => {
  ...
  useEffect(() => {
    ...
    const handleResize = debounce(() => {
      if (chartRef.current) {
        // Force re-render by updating a state that triggers chart re-render
  ...
  const renderBrush = useCallback(() => {
    ...
    const brush = d3
      ...
      .on('brush', () => {
        // Brush event handler - no action needed during brushing
      })
      .on('end', (event) => {
  ...
  }, [brushRef, dateRange, theme, parseTime, sentimentData])
